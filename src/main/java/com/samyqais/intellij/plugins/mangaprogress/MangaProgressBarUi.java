...
public class MangaProgressBarUi extends BasicProgressBarUI {
    private static final String DEBUGGING_ENV_VAR = "MANGA_PICTURE_PROGRESS_DEBUG";

    private final MangaPicture mangaPicture;
    ...
    public MangaProgressBarUi(final MangaPicture mangaPicture) {
        this(mangaPicture,
    ...
    public MangaProgressBarUi(final MangaPicture mangaPicture,
        ...
        iconForward = MangaPictureResourceLoader.getIcon(mangaPicture);
        iconReversed = MangaPictureResourceLoader.getReversedIcon(mangaPicture);
    ...
    public static ComponentUI createUI(final JComponent c) {
        ...
        return new MangaProgressBarUi(MangaPicturePicker.get());
    }
    ...
    public Dimension getPreferredSize(final JComponent c) {
        int height = restrictMaxHeight.get() ? Math.min(maxHeight.get(), mangaPicture.getHeight())
                : mangaPicture.getHeight();
    ...
    private void paint(final Graphics g, final JComponent c, final boolean determinate) {
        ...
        drawMangaPictureIcon(amountFull, graphics2D, rectangle);
    ...
    private void setToolTipText() {
        if (addToolTips.get()) {
            progressBar.setToolTipText(mangaPicture.getNameWithNumber());
        }
    }
    ...
    private void drawMangaPictureIcon(final int amountFull, final Graphics2D graphics2D, final Shape clip) {
        if (!drawSprites.get()) {
            return;
        }
        ...
        if (icon != null) {
            icon.paintIcon(progressBar,
                    graphics2D,
                    amountFull + (velocity >= 0 ?
                            JBUI.scale(scaleToHeightRestrictions(mangaPicture.getXShift())) :
                            JBUI.scale(-icon.getIconWidth() - scaleToHeightRestrictions(mangaPicture.getXShift()))
                    ),
                    JBUI.scale(scaleToHeightRestrictions(mangaPicture.getYShift())));
        }
    ...
    private int scaleToHeightRestrictions(final int value) {
        if (restrictMaxHeight.get() && mangaPicture.getHeight() > maxHeight.get()) {
            return Math.round(((float) maxHeight.get() / mangaPicture.getHeight()) * value);
        }
        if (restrictMinHeight.get() && mangaPicture.getHeight() < minHeight.get()) {
            return Math.round(((float) minHeight.get() / mangaPicture.getHeight()) * value);
        }
        return value;
    }
    ...
    private static <T> Supplier<T> safeGetFromState(final Function<MangaProgressState, T> getter,
                                                    final T defaultIfStateNull) {
        return () -> Optional.ofNullable(MangaProgressState.getInstance())
                .map(getter)
